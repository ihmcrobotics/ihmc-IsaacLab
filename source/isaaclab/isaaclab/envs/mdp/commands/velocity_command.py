...
if TYPE_CHECKING:
    ...
    from .commands_cfg import NormalVelocityCommandCfg, UniformVelocityCommandCfg, UniformVelocityPlusHeightCommandCfg
...
class UniformVelocityPlusHeightCommand(CommandTerm):
    r"""Command generator that generates a velocity command in SE(2) from uniform distribution.
    ...
    cfg: UniformVelocityPlusHeightCommandCfg
    """The configuration of the command generator."""

    def __init__(self, cfg: UniformVelocityPlusHeightCommandCfg, env: ManagerBasedEnv):
        """Initialize the command generator.
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        # -- lin pos z - height of pelvis off ground
        self.command_b[env_ids, 3] = r.uniform_(*self.cfg.ranges.lin_pos_z)
        # heading target
        if self.cfg.heading_command:
            self.heading_target[env_ids] = r.uniform_(*self.cfg.ranges.heading)
            # update heading envs
    ...
    def _update_command(self):
        ...
        if self.cfg.heading_command:
            ...
            # compute angular velocity
            heading_error = math_utils.wrap_to_pi(self.heading_target[env_ids] - self.robot.data.heading_w[env_ids])
            self.command_b[env_ids, 2] = torch.clip(
                self.cfg.heading_control_stiffness * heading_error,
                min=self.cfg.ranges.ang_vel_z[0],
                max=self.cfg.ranges.ang_vel_z[1],
            )
        ...
        # TODO: check if conversion is needed
        standing_env_ids = self.is_standing_env.nonzero(as_tuple=False).flatten()
        self.command_b[standing_env_ids, :2] = 0.0
